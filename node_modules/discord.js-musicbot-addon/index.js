...
module.exports = function(client, options) {
  ...
  class Music {
    constructor(client, options) {
      ...
      this.botPrefix = (options && options.prefix) || 'ash ';
  ...
  musicbot.play = (msg, suffix) => {
    ...
    msg.channel.send(musicbot.note('search', `Ищу: \`${searchstring}\``))
      .then(response => {
        if (searchstring.startsWith('http') && searchstring.includes("list=")) {
          ...
          ypi.playlistInfo(musicbot.youtubeKey, playid, function(playlistItems) {
            ...
            newItems.forEach(video => {
              ...
              if (ran == newItems.length) {
                if (index = 0) msg.channel.send(musicbot.note('fail', `Не могу включить эту песню`))
                else if (index == 1) msg.channel.send(musicbot.note('note', `Очередь из 1 песни`));
                else if (index > 1) msg.channel.send(musicbot.note('note', `Песен: ${index}`));
              }
            });
          });
        } else {
          search.search(searchstring, {
            ...
            .then(searchResult => {
              if (!searchResult.totalResults || searchResult.totalResults === 0) return response.edit(musicbot.note('fail', 'Не могу найти песню.'));
  ...
  musicbot.search = (msg, suffix) => {
    musicbot.dInvoker(msg);
    // Make sure the user is in a voice channel.
    if (msg.member.voiceChannel === undefined) return msg.channel.send(musicbot.note('fail', `Ты не в войсе.`));

    // Make sure the suffix exists.
    if (!suffix) return msg.channel.send(musicbot.note('fail', 'Ошибка'));

    // Get the queue.
    const queue = musicbot.getQueue(msg.guild.id);

    // Check if the queue has reached its maximum size.
    if (queue.length >= musicbot.maxQueueSize) return msg.channel.send(musicbot.note('fail', 'Лимит песен превышен.'));
    ...
    msg.channel.send(musicbot.note('search', `Ищу: \`${searchstring}\``))
      .then(response => {
        search.search(searchstring, {
          ...
          .then(searchResult => {
            if (!searchResult.totalResults || searchResult.totalResults === 0) return response.edit(musicbot.note('fail', 'Ошибка'));

            const startTheFun = async (videos, max) => {
              const embed = new Discord.RichEmbed();
              embed.setTitle(`Выбери песню`);
              embed.setColor(0x27e33d);
              var index = 0;
              ...
              embed.setFooter(`Поиск: ${msg.author.username}`, msg.author.displayAvatarURL);
              msg.channel.send({
                  embed
                })
                .then(firstMsg => {
  ...
  musicbot.skip = (msg, suffix) => {
    ...
    if (voiceConnection === null) return msg.channel.send(musicbot.note('fail', 'Ни одна песня не играет.'));

    // Get the queue.
    const queue = musicbot.getQueue(msg.guild.id);

    if (!musicbot.canSkip(msg.member, queue)) return msg.channel.send(musicbot.note('fail', `Ты не можешь скипать.`))
      .then((response) => {
    ...
    try {
      ...
      const nerr = e.toString()
        .split(':');
      return msg.channel.send(musicbot.note('fail', `Ошибка!\n\`\`\`\n${nerr[0]}: ${nerr[1]}\n\`\`\``));
    };

    if (first) return msg.channel.send(musicbot.note('note', 'Скипнуто **' + toSkip + '**!'));
    msg.channel.send(musicbot.note('note', 'Скипнуто **' + toSkip + '**!'));
  }
  ...
  musicbot.queue = (msg, suffix) => {
    ...
    // Get the queue.
    const queue = musicbot.getQueue(msg.guild.id);
    if (queue.length === 0) return msg.channel.send(musicbot.note(`note`, `Очередь пуста.`));
    let text;
    // Get the queue text.
    ...
    if (msg.channel.permissionsFor(msg.guild.me)
      ...
      if (songNum > 0) {
        ...
        embed.setAuthor(`Играет песня #${suffix}`, client.user.avatarURL);
        ...
        embed.setColor(0x27e33d);
        if (musicbot.requesterName && reqMem) embed.setFooter(`Заказал: ${reqMem.username}`, queue[songNum].requesterAvatarURL);
        if (musicbot.requesterName && !reqMem) embed.setFooter(`Заказал: \`Неизвестный пользователь (id: ${queue[songNum].requester})\``, queue[songNum].requesterAvatarURL)
        msg.channel.send({
          embed
        });
      } else {
        ...
        if (musicbot.enableQueueStat) {
          ...
          if (voiceConnection !== null) {
            const dispatcher = voiceConnection.player.dispatcher;
            queueStatus = dispatcher.paused ? 'Пауза' : 'Playing';

            embed.setAuthor(`Очередь (${queueStatus})`, client.user.avatarURL);
          } else {
            embed.setAuthor(`Очередь`, client.user.avatarURL);
          }
        }
        ...
        try {
          ...
          embed.setColor(0x27e33d);
          embed.setFooter(`Всего песен: ${queue.length}`, msg.author.displayAvatarURL);
        } catch (e) {
          console.log(e.stack);
        };
      ...
      try {
        if (musicbot.requesterName) {
          text = queue.map((video, index) => (
              (index + 1) + ': ' + video.title + ' | Заказал ' + client.users.get(video.requester)
              .username
            ))
            .join('\n');
        } else {
        ...
        if (text.length > 1900) {
          ...
          if (otherText.length > 1900) {
            msg.channel.send(musicbot.note('wrap', 'Очередь (' + queueStatus + '):\n' + "Лимит..."));
          } else {
            if (musicbot.enableQueueStat) {
              // Get the status of the queue.
              let queueStatus = 'Очередь остановлена';
              const voiceConnection = client.voiceConnections.find(val => val.channel.guild.id == msg.guild.id);
              if (voiceConnection !== null) {
                const dispatcher = voiceConnection.player.dispatcher;
                queueStatus = dispatcher.paused ? 'Пауза' : 'Играет';
              }

              // Send the queue and status.
              msg.channel.send(musicbot.note('wrap', 'Играет (' + queueStatus + '):\n' + newText));
              msg.channel.send(musicbot.note('wrap', 'Играет (2) (' + queueStatus + '):\n' + otherText));
            } else {
              msg.channel.send(musicbot.note('wrap', 'Играет:\n' + newText));
              msg.channel.send(musicbot.note('wrap', 'Играет (2):\n' + otherText));
            }
          };
        } else {
          if (musicbot.enableQueueStat) {
  ...
  musicbot.np = (msg, suffix) => {
    ...
    // Get the voice connection.
    const voiceConnection = client.voiceConnections.find(val => val.channel.guild.id == msg.guild.id);
    if (voiceConnection === null) return msg.channel.send(musicbot.note('fail', 'Ошибка'));
    const dispatcher = voiceConnection.player.dispatcher;
    const queue = musicbot.getQueue(msg.guild.id);
    if (msg.channel.permissionsFor(msg.guild.me)
      .has('EMBED_LINKS')) {
      const embed = new Discord.RichEmbed();
      try {
        embed.setAuthor('Сейчас игрет', client.user.avatarURL);
        ...
        if (musicbot.requesterName && resMem) embed.setFooter(`Заказал ${client.users.get(queue[0].requester).username}`, queue[0].requesterAvatarURL);
        if (musicbot.requesterName && !resMem) embed.setFooter(`Заказал \`UnknownUser (ID: ${queue[0].requester})\``, queue[0].requesterAvatarURL);
        msg.channel.send({
          embed
        });
      } catch (e) {
      ...
      try {
        var songTitle = queue[0].title.replace(/\\/g, '\\\\')
          ...
          .replace(/~/g, '\\~')
          .replace(/`/g, '\\`');
        msg.channel.send(`Сейчас играет: **${songTitle}**\nЗаказал: ${client.users.get(queue[0].requester).username}`)
      } catch (e) {
        console.log(`[${msg.guild.name}] [npCmd] ` + e.stack);
      };
    }
  ...
  musicbot.leave = (msg, suffix) => {
    ...
    if (musicbot.isAdmin(msg.member) && musicbot.anyoneCanLeave === true) {
      ...
      msg.channel.send(musicbot.note('note', 'Пока!'));
    } else {
      const chance = Math.floor((Math.random() * 100) + 1);
      if (chance <= 10) return msg.channel.send(musicbot.note('fail', `Я не могу это сделать, ${msg.author.username}.`))
      else return msg.channel.send(musicbot.note('fail', 'Только админы могут это сделать.'));
    }
  }
  ...
  musicbot.resume = (msg, suffix) => {
    ...
    if (!musicbot.isAdmin(msg.member))
      return msg.channel.send(musicbot.note('fail', 'Только для админов.'));

    // Resume.
    msg.channel.send(musicbot.note('note', 'Продолжаем слушать.'));
    const dispatcher = voiceConnection.player.dispatcher;
  ...
  musicbot.volume = (msg, suffix) => {
    ...
    // Get the voice connection.
    const voiceConnection = client.voiceConnections.find(val => val.channel.guild.id == msg.guild.id);
    if (voiceConnection === null) return msg.channel.send(musicbot.note('fail', 'Музыка не играет.'));

    // Get the queue.
    const queue = musicbot.getQueue(msg.guild.id);

    if (!musicbot.canAdjust(msg.member, queue))
      return msg.channel.send(musicbot.note('fail', 'Это для админов.'));

    // Get the dispatcher
    const dispatcher = voiceConnection.player.dispatcher;

    if (suffix > 200 || suffix < 0) return msg.channel.send(musicbot.note('fail', 'Неправильная громкость!'))
      .then((response) => {
        response.delete(5000);
      });
  ...
  musicbot.loop = (msg, suffix) => {
    ...
    const voiceConnection = client.voiceConnections.find(val => val.channel.guild.id == msg.guild.id);
    if (voiceConnection === null) return msg.channel.send(musicbot.note('fail', 'Сейчас музыка не играет.'));
  ...
  musicbot.set = (msg, suffix, args) => {
    if (!musicbot.isAdmin(msg.member)) return msg.channel.send(musicbot.note('fail', `Это для админов!`));
    const valid = ["prefix"];
    if (!args[0]) return msg.channel.send(musicbot.note('note', `Правильная настройка: ${valid.join(", ")}.`));
    if (!valid.includes(args[0])) return msg.channel.send(musicbot.note('fail', 'Неправильная настройка!'));
    if (args[0] && args[0] == "prefix") {
      const newPrefix = args.slice(1, args.length)
        .join(" ");
      if (!newPrefix || newPrefix.legnth < 1) return msg.channel.send(musicbot.note('fail', 'Ошибка префикса!'));
  ...
  musicbot.executeQueue = (msg, queue) => {
    ...
    if (queue.length === 0) {
      msg.channel.send(musicbot.note('note', 'Плейлист проигран.'));
    ...
    new Promise((resolve, reject) => {
        ...
        if (voiceConnection === null) {
          ...
          if (msg.member.voiceChannel && msg.member.voiceChannel.joinable) {
            ...
            msg.channel.send(musicbot.note('fail', 'Я не могу зайти на этот канал'))
            reject();
          } else {
